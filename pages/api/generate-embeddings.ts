...
export default async function handle(
  ...
  if (method === "POST") {
    ...
    //const  urls  = JSON.stringify(body);
    const { urls } = body;
    console.log("test" + JSON.stringify(urls));
    //const documents = await getDocuments(urls);
    console.log("hello:"+urls + typeof urls);

    //for (const { url, body } of documents) {
      //const input = urls[0].replace(/\n/g, " ");
      const input = urls.toString();
      console.log("\input : \n", input);
